...
for trial = 1 : length(trials)
    ...
    try
        if length(times.lLEDon) == 0 || length(times.rLEDon) == 0
            ...
            lst = [1,(diff(times.lLEDon)>Fs*0.05)];
            lstl = (lst(1:length(times.lLEDon)-1)>0);
            times.lLEDstart = times.lLEDon(lstl);
            %results in three specific time points for this LED
            
            rst = [1,(diff(times.rLEDon)>Fs*0.05)];
            rstl = (rst(1:length(times.rLEDon)-1)>0);
            times.rLEDstart = times.rLEDon(rstl);
            ...
            delay = 0.05;
            windowSize = round(Fs * delay); %creates our time interval by taking the 20k
            ...
            for l = 1:length(times.lLEDstart)
                ...
                %windowSize = round(Fs*delay); % window size in samples
            ...
            ui.LrasterStack = reshape(ui.Lraster,length(ui.Lraster)/length(times.Lrasterlight),length(times.Lrasterlight));
            ui.RrasterStack = reshape(ui.Rraster,length(ui.Rraster)/length(times.Rrasterlight),length(times.Rrasterlight));
            ...
            %     figure('Visible','off');
            tLras = t.Lraster;
            Lras = ui.LrasterStack+Lstack-1;
            plot(tLras,Lras);
            hold on;
            %             length(times.Lrasterlight)
            line([delay delay], [0 length(times.Lrasterlight)], 'Color', 'k', 'LineWidth',2)
            ...
            %     figure('Visible','off');
            tRras = t.Rraster;
            Rras = ui.RrasterStack+Rstack-1;
            plot(tRras,Rras);
            hold on;
            line([delay delay], [0 length(times.Rrasterlight)], 'Color', 'k', 'LineWidth',2)
            hold off;
            ylabel 'trial number';
            xlabel 'time (s)';
            xlim([0 2*delay]);
